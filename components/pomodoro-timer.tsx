...
export function PomodoroTimer({
  ...
  return (
    <Card 
      className={fullScreen ? "h-full border-0 shadow-none rounded-none flex flex-col bg-transparent" : ""}
      data-testid="pomodoro-timer"
    >
      <CardHeader className={`pb-2 flex flex-row items-center justify-between ${fullScreen ? 'pt-2' : ''}`}>
        <div className="w-9"></div>
        <CardTitle className="text-lg sm:text-xl text-center flex-1">
          {t("pomodoroTimer")}
        </CardTitle>
        <Button 
          variant="ghost" 
          ...
          data-testid="pomodoro-settings-button"
        >
      ...
      <CardContent className={`pt-0 ${fullScreen ? 'flex flex-col items-center justify-center flex-1' : ''}`}>
        ...
        <div className={`w-full flex flex-col items-center ${fullScreen ? 'flex-1 justify-center' : 'mt-4 sm:mt-6'}`}>
          <div 
            className={`${fullScreen ? 'text-6xl sm:text-7xl mb-6' : 'text-4xl sm:text-5xl'} font-bold tabular-nums ${timerTextColorClass}`}
          ...
          <div className={`w-full ${fullScreen ? 'max-w-[85%] mx-auto mb-8' : 'mt-4 mb-6'}`}>
            <Progress 
              value={getProgress()} 
              className={`${fullScreen ? 'h-3' : 'h-2'} w-full`}
              data-testid="pomodoro-progress-bar"
            />
          </div>
          ...
          <div 
            className={`text-xs sm:text-sm text-muted-foreground ${fullScreen ? 'mt-2' : 'mt-6'}`}
