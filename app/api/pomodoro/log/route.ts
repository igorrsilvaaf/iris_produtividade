...
export async function POST(request: Request) {
  try {
    ...
    const mode: string =
      typeof rawMode === "string" && rawMode.trim() !== "" ? rawMode : "work";
...
export async function GET(request: Request) {
  try {
    ...
    const taskIds = Array.from(
      new Set(
        pomodoroLogs
          .filter((log) => log.taskId !== null)
          .map((log) => Number(log.taskId))
      )
    );

    let tasksMap = new Map<number, { id: number; title: string }>();
    if (taskIds.length > 0) {
    ...
    const pomodoroLogsWithTask = pomodoroLogs.map((log) => ({
      ...log,
      task: log.taskId ? tasksMap.get(Number(log.taskId)) ?? null : null,
    }));
    ...
    return new NextResponse(
      JSON.stringify({
        success: true,
        pomodoroLogs: pomodoroLogsWithTask,
        pagination: {
          total,
          page,
          limit,
