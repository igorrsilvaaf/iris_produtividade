...
export async function POST(request: NextRequest) {
  try {
    ...
    const filteredCustomColumns = Array.isArray(customColumns)
      ? customColumns
      : [];
    ...
    "Params:", params;
    const sql = {
      query: (q: string, p: any[]) => prisma.$queryRawUnsafe<any[]>(q, ...p),
    };
    try {
        ...
        try {
          ...
          if (filteredLabelIds.length > 0) {
            ...
            if (
              Array.isArray(tasks) &&
              tasks.length > 0 &&
              tasks.some((t) => t.labels && t.labels.length > 0)
            ) {
              const filteredTasks = tasks.filter((task) => {
                ...
                return task.labels.some(
                  (label: any) =>
                    label &&
                    label.id &&
                    filteredLabelIds.includes(String(label.id))
                );
              });
    ...
    if (error instanceof Error && error.message === "Query timeout") {
      return NextResponse.json(
        {
          error:
            "A consulta levou muito tempo para ser processada. Por favor, reduza o número de filtros ou o período do relatório.",
        },
        { status: 408 }
      );
    }
