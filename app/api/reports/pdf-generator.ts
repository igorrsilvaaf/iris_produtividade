...
export function generatePDF(data: ReportData): Buffer {
  try {
    ...
    data.items = data.items.map((item) => {
      ...
      if (itemWithExtras.labels) {
        ...
        if (typeof itemWithExtras.labels === "string") {
          try {
            itemWithExtras.labels = JSON.parse(
              itemWithExtras.labels as unknown as string
            );
          } catch (e) {
    ...
    if (data.items.length > 0) {
      ...
      const tasksWithPoints = data.items.filter(
        (task) => task.points && task.points > 0
      );
      const totalPoints = tasksWithPoints.reduce(
        (sum, task) => sum + (task.points || 0),
        0
      );
      const avgPoints =
        tasksWithPoints.length > 0
          ? Math.round((totalPoints / tasksWithPoints.length) * 10) / 10
          : 0;

      // Estatísticas de tempo estimado
      const tasksWithTime = data.items.filter(
        (task) => task.estimated_time && task.estimated_time > 0
      );
      const totalEstimatedTime = tasksWithTime.reduce(
        (sum, task) => sum + (task.estimated_time || 0),
        0
      );
      const avgEstimatedTime =
        tasksWithTime.length > 0
          ? Math.round(totalEstimatedTime / tasksWithTime.length)
          : 0;
      ...
      if (totalEstimatedTime > 0) {
        ...
        doc.setFontSize(12);
        doc.setTextColor(colors.primary);
        doc.text(formatEstimatedTime(totalEstimatedTime), 212, 47, {
          align: "center",
        });
        doc.setFontSize(8);
        doc.setTextColor(colors.text);
      ...
      let xPosition =
        totalEstimatedTime > 0 ? 236 : totalPoints > 0 ? 192 : 160;
    ...
    if (data.filters) {
      const hasProjectFilters =
        data.filters.projectIds && data.filters.projectIds.length > 0;
      const hasLabelFilters =
        data.filters.labelIds && data.filters.labelIds.length > 0;
      const hasPriorityFilters =
        data.filters.priorities && data.filters.priorities.length > 0;
      ...
      if (hasProjectFilters || hasLabelFilters || hasPriorityFilters) {
        ...
        if (hasProjectFilters) {
          ...
          if (data.filters.projectIds!.length > 10) {
            projectText += `Todos (${
              data.filters.projectIds!.length
            } selecionados)`;
          } else {
        ...
        if (hasPriorityFilters) {
          ...
          if (data.filters.priorities!.length >= 5) {
            ...
            priorityNames = data.filters
              .priorities!.map((p) => priorityLabels[parseInt(p)] || p)
              .join(", ");
          }
      ...
      const rowData: Record<string, any> = {
        ...
        priority:
          task.priority !== undefined
            ...
            : "-",
        completed:
          task.completed !== undefined ? (task.completed ? "Sim" : "Não") : "-",
        project_name: taskWithExtras.project_name || "-",
        labels: taskWithExtras.labels
          ? taskWithExtras.labels.map((label) => label.name).join(", ")
          : "-",
        ...
        pomodoro_minutes:
          typeof taskWithExtras.pomodoro_minutes === "number"
            ? `${taskWithExtras.pomodoro_minutes} min`
            : "-",
